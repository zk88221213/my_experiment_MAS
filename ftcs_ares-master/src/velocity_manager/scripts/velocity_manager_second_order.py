...
def velocity_manager():
    ...
    rospy.Subscriber("control_msg", Accel, OnControlMsg)
    rospy.Subscriber("base_pose_ground_truth", Odometry, OnGroundTruth)
    prev_e = 0;
    cur_time = rospy.get_time() - 0.05
    #设置循环的频率
    rate = rospy.Rate(20)
    while not rospy.is_shutdown():  
        ...
        prev_e = e;
        # 对控制输入进行积分
        expect_velocity.linear.x += control_msg_latest.linear.x * (cur_time - prev_time)
        expect_velocity.linear.y += control_msg_latest.linear.y * (cur_time - prev_time)

        #求出实际运动速度
        cmd_vel.linear.x=flowfield_velocity_latest.linear.x + expect_velocity.linear.x
        cmd_vel.linear.y=flowfield_velocity_latest.linear.y + expect_velocity.linear.y
